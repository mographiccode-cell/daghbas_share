...
function Audit({token,t}) {const [data,setData]=useState([]);const load=()=>api('/api/audit',{token}).then(setData);useEffect(()=>{load()},[]);return <><div className="page-head"><h2>{t.audit}</h2><button className="secondary" onClick={load}>{t.refresh}</button></div><section className="panel"><table><thead><tr><th>{t.event}</th><th>{t.details}</th><th>{t.date}</th></tr></thead><tbody>{data.map(x=><tr key={x.id}><td>{x.event_type}</td><td><code>{JSON.stringify(x.details)}</code></td><td>{new Date(x.created_at).toLocaleString()}</td></tr>)}</tbody></table></section></>}
