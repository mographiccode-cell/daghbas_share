...
class LocalShareService extends ChangeNotifier {
  ...
  static const String _securePeersKey = 'localshare_peers_v3';
  ...
  String get receivePolicyLabel {
    ...
    if (Platform.isWindows) return 'الاستلام مؤقت حتى تختار حفظ دائم';
    return receiveDirectory.path;
  }
  ...
  Future<void> init() async {
    try {
      ...
      deviceName = _prefs!.getString('device_name') ?? _defaultDeviceName();
      await _prefs!.setString('device_name', deviceName);

      // Protocol v3 intentionally invalidates older trust material.
      await _secureStorage.delete(key: 'localshare_peers_v2');
  ...
  Future<Directory> _prepareReceiveDirectory() async {
    if (Platform.isWindows) {
      ...
      final session = Directory(
        '${root.path}${Platform.pathSeparator}session-${deviceId.substring(0, min(8, deviceId.length))}-$pid',
      );
    ...
    if (Platform.isAndroid) {
      ...
      final staging = Directory(
        '${base.path}${Platform.pathSeparator}incoming-staging',
      );
    ...
    final folder = Directory(
      '${support.path}${Platform.pathSeparator}LocalShareIncoming',
    );
    await folder.create(recursive: true);
    return folder;
  }

  Future<void> _cleanupTemporaryReceiveDirectory() async {
  ...
  void _onDiscoveryEvent(RawSocketEvent event) {
    ...
    while ((datagram = _discoverySocket!.receive()) != null) {
      ...
      if (parts.length != 5 || parts[0] != 'LOCALSHARE3') continue;
      final type = parts[1];
      final id = parts[2];
  ...
  void _sendDiscovery(String type, InternetAddress address) {
    ...
    final data =
        'LOCALSHARE3|$type|$deviceId|${Uri.encodeComponent(deviceName)}|$serverPort';
    try {
      socket.send(utf8.encode(data), address, discoveryPort);
    } catch (_) {}
  }
  ...
  void _removeStaleDevices() {
    ...
    if (before != _discovered.length) notifyListeners();
  }
  ...
  Future<void> _handleRequest(HttpRequest request) async {
    ...
    try {
      ...
      if (request.method == 'POST' && request.uri.path == '/message3') {
        await _handleEncryptedMessage(request);
        return;
      }
      if (request.method == 'POST' && request.uri.path == '/upload3') {
  ...
  Future<void> _handleEncryptedMessage(HttpRequest request) async {
    final data = await _readJsonBody(request);
    final sourceId = '${data['deviceId'] ?? ''}'.trim();
    final timestamp = (data['timestamp'] as num?)?.toInt() ?? 0;
    final nonce = '${data['nonce'] ?? ''}'.trim();
    final box = '${data['box'] ?? ''}'.trim();
    final peer = _paired[sourceId];
    if (peer == null ||
        !_timestampIsFresh(timestamp) ||
        !_crypto.isValidB64Length(nonce, 12) ||
        box.isEmpty ||
        box.length > 16384) {
      throw const _RequestException(HttpStatus.unauthorized, 'unauthorized');
    }

    final replayKey = 'msg:$sourceId:$nonce';
    if (_recentNonces.containsKey(replayKey)) {
      throw const _RequestException(HttpStatus.conflict, 'replay_rejected');
    }

    Map<String, dynamic> payload;
    try {
      payload = await _crypto.decryptPayload(
        sharedKey: _peerKey(peer),
        senderId: sourceId,
        receiverId: deviceId,
        timestamp: timestamp,
        nonceId: nonce,
      ...
      throw const _RequestException(HttpStatus.unauthorized, 'invalid_message');
    }

    final id = '${payload['id'] ?? ''}';
    final kindRaw = '${payload['kind'] ?? ''}';
    ...
    _jsonResponse(request, HttpStatus.ok, {'ok': true, 'proof': ack});
  }
  ...
  Future<void> _handleEncryptedUpload(HttpRequest request) async {
    ...
    final replayKey = 'file:$sourceId:$transferNonce';
    if (_recentNonces.containsKey(replayKey)) {
      throw const _RequestException(HttpStatus.conflict, 'replay_rejected');
    }
    ...
    try {
      ...
      while (remaining > 0) {
        ...
        chunkIndex++;
        transfer.progress = total == 0 ? 1 : (received / total).clamp(0.0, 1.0);
        final now = DateTime.now();
      ...
      String finalLocation = destination.path;
      var temporary = Platform.isWindows;
  ...
  Future<void> probeIp(String rawIp) async {
    var ip = rawIp.trim().replaceFirst(RegExp(r'^https?://'), '');
    ip = ip.split('/').first;
  ...
  Future<void> forgetPeer(String id) async {
    _paired.remove(id);
    _messages.removeWhere((message) => message.peerId == id);
    await _savePeers();
    notifyListeners();
  }

  Future<void> sendChat(Peer peer, String rawText) async {
  ...
  Future<String> savePermanently(ChatMessage message) async {
    if (!Platform.isWindows ||
        !message.isFile ||
        message.localPath == null ||
        message.localPath!.isEmpty) {
      return message.localPath ?? '';
    }
    final source = File(message.localPath!);
    if (!await source.exists()) {
      throw const FileSystemException('الملف المؤقت لم يعد موجودًا');
    }
    final target = await FilePicker.platform.saveFile(
      dialogTitle: 'حفظ الملف بشكل دائم',
      fileName: message.fileName ?? source.uri.pathSegments.last,
    );
    if (target == null || target.isEmpty) return '';
    final saved = await source.copy(target);
    try {
      await source.delete();
    } catch (_) {}
    message.localPath = saved.path;
  ...
  Future<void> openFile(ChatMessage message) async {
    ...
    if (Platform.isAndroid) {
      await _native.invokeMethod<void>('openUri', {'uri': path});
      return;
    }
    if (Platform.isWindows) {
  ...
  Future<void> openLink(String rawUrl) async {
    ...
    if (uri == null ||
        ...
        uri.host.isEmpty) {
      throw const FormatException('الرابط غير صالح');
    }
    if (Platform.isAndroid) {
      await _native.invokeMethod<void>('openUri', {'uri': uri.toString()});
      return;
    }
    if (Platform.isWindows) {
  ...
  Future<bool> _verifyPeerEndpoint(Peer peer, String ip, int port) async {
    ...
    try {
      ...
      return _crypto.constantTimeEquals(expected, '${decoded['proof'] ?? ''}');
    } catch (_) {
      return false;
    } finally {
      client.close(force: true);
  ...
  void dispose() {
    ...
    unawaited(_cleanupTemporaryReceiveDirectory());
    super.dispose();
  }
}
